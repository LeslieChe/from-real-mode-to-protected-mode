...
put_string:                                 ;显示0终止的字符串并移动光标 
		...
		;|          |
		 ...
		 mov bx,[ebp+12*4]   ;取得 @2段选择子 
		 arpl bx,ax			 ;调整段选择子的 RPL
		 mov ds,bx

		 mov ebx,[ebp+11*4] ;取得 @3段内偏移
		 mov ch,[ebp+13*4]  ;取得 @1属性
...
put_hex_dword:                              ;在当前光标处以十六进制形式显示
		...
		;|                   |
		 ...
		 mov edx,[ebp+11*4]   ;提取 @2，要打印的寄存器的值
		 mov ch,0x0b          ;字符属性是浅青色
  .p_name:                    ;这个循环是为了打印第 @1
		 mov cl,[ebp+12*4]
		 or cl,cl
		 jz .ok
